...
func Get_convertHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_convertTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_convert",
		mcp.WithDescription("Convert a currency amount to multiple other currencies"),
		...
		mcp.WithString("decimal_places", mcp.Description("This parameter can be used to specify the number of decimal places included in the output. If an amount is not specified then 12 is assumed.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_convertHandler(cfg),
	}
}
